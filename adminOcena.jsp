<%-- 
    Document   : adminOcene
    Created on : Sep 24, 2020, 5:27:19 PM
    Author     : Super User
--%>
    ...
    <table class="table table-striped table-dark">
        ...
        <%
            try {
                ...
                String sql = "SELECT * FROM fakultetzp.ocena";
                resultSet = statement.executeQuery(sql);
                while (resultSet.next()) {
        %>
        <tr>
            <td><%=resultSet.getString("oce_id")%></td>
            <td><%=resultSet.getString("oce_ocena")%></td>
            <td><%=resultSet.getString("std_id")%></td>
            <td><%=resultSet.getString("pre_id")%></td>
            <td><%=resultSet.getString("pro_id")%></td>
            <td><a href="updateoce.jsp?id=<%=resultSet.getString("oce_id")%>"><button type="button" class="btn btn-outline-primary">AZURIRAJ</button></a></td>
            <td><button type="button" id="<%=resultSet.getString("oce_id")%>" class="btn btn-outline-primary delete">IZBRISI</button></td>
        </tr>
        <%
                }
                connection.close();
