<%-- 
    Document   : updateoce
    Created on : Sep 24, 2020, 5:47:12 PM
    Author     : Super User
--%>
...
<%
    try {
        ...
        String sql = "SELECT * FROM fakultetzp.ocena WHERE oce_id=" + id;
...
<body style="background-color:lightgray; font: black">
    ...
    <form method="post">
        <table class="table table-striped table-dark">
            ...
            <tbody>
            <td scope="row"><input type="hidden" name="oce_id" id="oce_id" value="<%=resultSet.getString("oce_id")%>"></td>
            <td scope="row"><input type="text" name="oce_ocena" id="oce_ocena" value="<%=resultSet.getString("oce_ocena")%>"></td>
            <td scope="row"><input type="text" name="std_id" id="std_id" value="<%=resultSet.getString("std_id")%>"></td>
            <td scope="row"><input type="text" name="pre_id" id="pre_id" value="<%=resultSet.getString("pre_id")%>"></td>
            <td scope="row"><input type="text" name="pro_id" id="pro_id" value="<%=resultSet.getString("pro_id")%>"></td>
            <td scope="row" name="azurirajo" id="azurirajo" value="AZURIRAJ"><button type="button" class="btn btn-outline-primary">AZURIRAJ</button></td>
            </tbody>
    ...
    <script>
        $(document).ready(function () {
            $("#azurirajo").click(function () {
                alert("succeess");
                $.ajax({
                    url: "updateprocessoce.jsp",
                    type: "post",
                    data: {
                        oce_id: $('#oce_id').val(),
